...
docker-bridge address instead of public_ip_intent. The root cause is pipeline
...
(common_qa). public_ip_intent's Adapt confidence for this exact phrasing
...
public_ip_intent's own (correct) answer never got a chance to run.

public_ip_intent.intent gives the mandated phrasings a padatious-family match that
...
class TestPublicIPDefaultPipeline(TestCase):
    ...
    def test_whats_my_public_ip_resolves_before_fallback_high(self):
        # public_ip_intent.intent (padatious-high) must claim this, not the
        # lower-tier Adapt public_ip_intent (adapt-medium, conf ~0.6).
    ...
    def test_whats_my_external_ip_resolves_before_fallback_high(self):
        self._assert_intent("what's my external ip address", "public_ip_intent")

    def test_bare_local_ip_question_unaffected(self):
        # negative: a plain local-ip question must still route to ip_intent,
        # not public_ip_intent.intent, on the same pipeline.
        matched = []
        handler = lambda msg: matched.append(msg)
        self.minicroft.bus.on(f"{SKILL_ID}:public_ip_intent", handler)
        try:
            ...
            self.minicroft.bus.remove(f"{SKILL_ID}:public_ip_intent", handler)
        self.assertFalse(
            matched, "\"what is my ip\" incorrectly routed to public_ip_intent.intent"
        )
